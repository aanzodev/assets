const DEFAULT_WISP = window.SITE_CONFIG?.defaultWisp ?? "wss://wispy.thewellnessconference.org/wisp/";
const WISP_SERVERS = [{ name: "Relic Network", url: "wss://wispy.thewellnessconference.org/wisp/" }];

// Initialize default  server if not set
if (!localStorage.getItem("SystemServer")) {
    localStorage.setItem("SystemServer", DEFAULT_WISP);
}
...
async function initializeWithBestServer() {
    ...
    const currentUrl = localStorage.getItem("SystemServer") || DEFAULT_WISP;
    ...
    if (best && best !== currentUrl) {
        ...
        localStorage.setItem("SystemServer", best);
...
async function getSharedConnection() {
    ...
    const basePath = getBasePath();
    const wispUrl = localStorage.getItem("SystemServer") ?? DEFAULT_WISP;
        ...
        <div class="browser-container">
            ...
            <div class="flex nav">
                ...
                <div class="address-wrapper">
                    <input class="bar" id="address-bar" autocomplete="off" placeholder="relic://newtab">
                ...
                <button id="wisp-settings-btn" title="System Settings"><i class="fa-solid fa-gear"></i></button>
...
function showIframeLoading(show, url = '') {
    ...
    // keep state for logic (skip button, loading flags, etc.)
    tab.loading = show;

    // DO NOT show any loading UI
    const loader = document.getElementById("loading");
    if (loader) loader.style.display = "none";

    // remove loading class so nothing visually changes
    tab.frame.frame.classList.remove('loading');

    // still update skip button logic (hidden always)
...
window.deleteCustomWisp = function (urlToDelete) {
    ...
    if (localStorage.getItem('SystemServer') === urlToDelete) {
...
function setWisp(url) {
    const oldUrl = localStorage.getItem('SystemServer');
    localStorage.setItem('SystemServer', url);

    if (oldUrl !== url) {
        const serverName = [...WISP_SERVERS, ...getStoredWisps()].find(s => s.url === url)?.name ?? 'Custom Server';
        notify('success', 'System Changed', `Switching to ${serverName}...`);
    }
        ...
        if ('serviceWorker' in navigator) {
            ...
            navigator.serviceWorker.addEventListener('message', (event) => {
                ...
                if (type === 'wispChanged') {
                    ...
                    localStorage.setItem("SystemServer", url);
                    notify('info', 'Autoswitched', `Now using ${name} because the previous server was slow or offline.`);
                } else if (type === 'wispError') {
                    console.error("SW reported Wisp Error:", event.data);
                    notify('error', 'System Error', message);
                }
            });
